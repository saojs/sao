...
module.exports = async (
  actions,
  { templateDir, generatorPath, outDir, context, config }
) => {
  for (const action of actions) {
    if (action.type === 'add' && action.files) {
      const stream = majo()
      stream.source(['!**/node_modules/**'].concat(action.files), {
        baseDir: path.resolve(generatorPath, templateDir)
      })
      ...
      const stream = majo()
      stream.source(action.files, { baseDir: outDir })
      stream.use(async ({ files }) => {
        await Promise.all(
          ...
          Object.keys(files).map(relativePath => {
            ...
            let result = action.handler(contents, relativePath)
            if (isJson) {
      ...
      await Promise.all(
        Object.keys(action.patterns).map(async pattern => {
          ...
          if (files.length > 1) {
            ...
            const from = files[0]
            const to = path.join(outDir, action.patterns[pattern])
