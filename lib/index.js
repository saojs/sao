...
module.exports = class SAO {
  ...
  normalizeOptions(options = {}) {
    ...
    return {
      ...
      skipCache: typeof options.skipCache === 'boolean' ? options.skipCache : process.env.NODE_ENV === 'test',
      logLevel
    }
  }
  ...
  static async mockPrompt(from, answers, subAnswers) {
    ...
    const sao = new SAO(Object.assign({
      outDir: path.join(
        require('os').tmpdir(),
  ...
  async generate({ from = this.options.from, baseDir, parentName, name } = {}) {
    ...
    if (config.prompts) {
      ...
      if (this.options.mock) {
        ...
        for (const prompt of prompts) {
          if (!Object.prototype.hasOwnProperty.call(this.answers, prompt.name)) {
