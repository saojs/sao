...
module.exports = co.wrap(function*(
  ...
  const showTip = () => {
    ...
    if (isNewFolder) {
      log.info(`Type \`cd ${chalk.yellow(path.relative(process.cwd(), folderPath))}\` to get started!`)
    }
  }
  ...
  if (projectConfig.template === 'handlebars') {
    kopyOptions.templateEngine = require('jstransformer-handlebars')
  } else if (typeof projectConfig.template === 'string') {
    kopyOptions.templateEngine = utils.requireAt(
      fromPath,
      `jstransformer-${projectConfig.template}`
    )
  } else {
    kopyOptions.templateEngine = projectConfig.template
  }
  ...
  if (projectConfig.prompts) {
    ...
    if (mockPromptData) {
      ...
      for (const prompt of kopyOptions.prompts) {
        ...
        if (typeof prompt.filter === 'function') {
          const res = prompt.filter(data[prompt.name], data)
          data[prompt.name] = res.then ? yield res : res
        }
        // Validation
        if (typeof prompt.validate === 'function') {
          let res = prompt.validate(data[prompt.name], data)
          res = res.then ? yield res : res
          if (!res) {
            throw new SAOError(`Validation failed at prompt: "${prompt.name}"`)
          }
        }
      }

      delete kopyOptions.prompts
      Object.assign(kopyOptions.data, data)
    }
  }
