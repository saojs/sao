...
module.exports = co.wrap(function * ({
  ...
  if (/^https?:\/\//.test(configFileName)) {
    ...
    projectConfig = yield configUtils.getConfig(fromPath, configFileName, template)
  }
      ...
      if (mockPromptData) {
        ...
        prompts.forEach(prompt => {
          if (Object.hasOwnProperty.call(mockPromptData, prompt.name)) {
            ...
            if (typeof prompt.validate === 'function' && !prompt.validate(data[prompt.name], data)) {
              throw new SAOError(`Validation failed at "${prompt.name}"`)
            }
          } else {
            data[prompt.name] = typeof prompt.default === 'function' ? prompt.default(data) : prompt.default
          }
        })
        prompts = undefined
      }
  ...
  return yield copy(path.join(fromPath, templateFolder), targetPath, {
    ...
    if (prompts) {
      for (const prompt of prompts) {
        if (prompt.store) {
          const answer = res.answers[prompt.name]
    ...
    if (!printLogs) return res

    const actionContext = Object.assign({}, res, {
      $,
      chalk,
      log,
      install: () => {
