...
module.exports = co.wrap(function * ({
  ...
  if (projectConfig) {
    ...
    if (projectConfig.prompts) {
      ...
      prompts = prompts.map(handlePrompt({targetFolder, template}))
    }
  }
  ...
  return yield copy(path.join(fromPath, templateFolder), targetFolder, {
    ...
    if (prompts) {
      for (const prompt of prompts) {
        if (prompt.store) {
          // todo: use only prompts answers instead data
          const newData = data[prompt.name]
          if (newData !== undefined) {
            promptStore.set(`${template}.${prompt.name}`, newData)
          }
        }
      }
    }
